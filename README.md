# Iconsource — Figma Icon Library Plugin
...
- `iconsource:icon` — the Iconify icon id (`<prefix>:<name>`)
- `iconsource:svgHash` — a hash of the SVG that was actually inserted
- `iconsource:version` — the source collection's version string, when Iconify exposes one (informational; the hash is what drives update detection)
...
Iconsource is developed and maintained by [Atropical AS](https://atropical.no).
